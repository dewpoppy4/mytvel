<?php 
  ...
  if(isset($_GET['fetch_tvels']))
  {
    ...
    $tvel_res = select("SELECT * FROM `tvels` WHERE `status`=? AND `removed`=?",[1,0],'ii');
    ...
    while($tvel_data = mysqli_fetch_assoc($tvel_res))
    {
      ...
      if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
      {
        $tb_query = "SELECT COUNT(*) AS `total_bookings` FROM `booking_order`
          WHERE booking_status=? AND tvel_id=?
          AND check_out > ? AND check_in < ?";

        $values = ['booked',$tvel_data['id'],$chk_avail['checkin'],$chk_avail['checkout']];
        $tb_fetch = mysqli_fetch_assoc(select($tb_query,$values,'siss'));

        if(($tvel_data['quantity']-$tb_fetch['total_bookings'])==0){
          continue;
        }
      }
      ...
      $fac_q = mysqli_query($con,"SELECT f.name, f.id FROM `facilities` f 
        INNER JOIN `tvel_facilities` tfac ON f.id = tfac.facilities_id 
        WHERE tfac.tvel_id = '$tvel_data[id]'");
      ...
      $fea_q = mysqli_query($con,"SELECT f.name FROM `features` f 
        INNER JOIN `tvel_features` tfea ON f.id = tfea.features_id 
        WHERE tfea.tvel_id = '$tvel_data[id]'");
      ...
      $thumb_q = mysqli_query($con,"SELECT * FROM `tvel_images` 
        WHERE `tvel_id`='$tvel_data[id]' AND `thumb`='1'");
      ...
      $rating_q = "SELECT AVG(rating) as avg_rating, COUNT(*) as total_reviews 
                   FROM rating_review WHERE tvel_id = '$tvel_data[id]'";
      ...
      $short_desc = '';
if(strlen($tvel_data['description']) > 350) {
    $short_desc = mb_substr($tvel_data['description'], 0, 350, 'UTF-8') . '...';
} else {
    $short_desc = $tvel_data['description'];
}
  ...
  <div class='card mb-4 border-0 shadow'>
    ...
    <img src='$tvel_thumb' 
      ...
      <div class='col-md-5 px-lg-3 px-md-3 px-0'>
        <h5 class='mb-3'>$tvel_data[name]</h5>
      ...
      <div class='col-md-2 mt-lg-0 mt-md-0 mt-4 text-center'>
        <a href='tvel_details.php?id=$tvel_data[id]' 
