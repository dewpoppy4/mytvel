...
<head>
  ...
  <title>สมัครสมาชิกผู้ประกอบการ</title>
...
<body class="bg-light">
  ...
  <div class="register-form text-center rounded bg-white shadow overflow-hidden">
    <form method="POST">
      <h4 class="bg-dark text-white py-3">สมัครสมาชิกผู้ประกอบการ</h4>
      <div class="p-4">
        <div class="mb-3">
      ...
      <div class="mb-3">
        <a href="entre_index.php" class="text-decoration-none">
  ...
  <?php 
    if(isset($_POST['register'])) {
      ...
      if($frm_data['entre_pass'] != $frm_data['confirm_pass']) {
        alert('error','รหัสผ่านไม่ตรงกัน!');
        exit;
      }

      // ตรวจสอบชื่อผู้ใช้ซ้ำ
      $query = "SELECT * FROM `entre_cred` WHERE `entre_name`=?";
      $values = [$frm_data['entre_name']];
      ...
      // เพิ่มข้อมูลผู้ประกอบการใหม่
      $query = "INSERT INTO `entre_cred`(`entre_name`,`entre_pass`) VALUES (?,?)";
